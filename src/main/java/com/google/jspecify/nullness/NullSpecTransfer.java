...
final class NullSpecTransfer extends CFAbstractTransfer<CFValue, NullSpecStore, NullSpecTransfer> {
  ...
  private AnnotatedTypeMirror typeWithTopLevelAnnotationsOnly(
    ...
    /*
     * In the case of a type-variable usage, we need not just the top-level annotation from the
     * usage but also any annotations on the bounds. To get that, we need to call the more
     * heavyweight getAnnotatedType instead of calling createType directly.
     *
     * My concern with calling getAnnotatedType in general has been infinite recursion:
     * getAnnotatedType tries to use dataflow, probably leading to a call back into the method we're
     * already in. Fortunately, it seems that we can get away with calling getAnnotatedType by
     * calling it on the corresponding type-parameter declaration, where dataflow presumably doesn't
     * matter.
     */
    AnnotatedTypeMirror type =
        node.getType().getKind() == TYPEVAR
            ? atypeFactory.getAnnotatedType(((TypeVariable) node.getType()).asElement()).asUse()
            : createType(node.getType(), atypeFactory, /*isDeclaration=*/ false);
